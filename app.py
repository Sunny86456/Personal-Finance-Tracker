...
def load_database_config():
    ...
    return {
        ...
        "DB_NAME": (parsed.path or "/finance_tracker").lstrip("/"),
    }
...
def last_n_months_overview(user_id, months=12):
    ...
    with db.cursor() as cur:
        cur.execute(
            """
            SELECT EXTRACT(YEAR FROM income_date) AS y,
                   EXTRACT(MONTH FROM income_date) AS m,
                   COALESCE(SUM(amount), 0) AS total
            FROM income
            WHERE user_id = %s
              AND income_date >= CURRENT_DATE - (%s * INTERVAL '1 month')
        ...
        cur.execute(
            """
            SELECT EXTRACT(YEAR FROM expense_date) AS y,
                   EXTRACT(MONTH FROM expense_date) AS m,
                   COALESCE(SUM(amount), 0) AS total
            FROM expenses
            WHERE user_id = %s
              AND expense_date >= CURRENT_DATE - (%s * INTERVAL '1 month')
...
def monthly_totals_current_year(user_id):
    ...
    with db.cursor() as cur:
        cur.execute(
            """
            SELECT EXTRACT(MONTH FROM income_date) AS m, COALESCE(SUM(amount), 0) AS total
            FROM income
            WHERE user_id = %s AND EXTRACT(YEAR FROM income_date) = %s
        ...
        cur.execute(
            """
            SELECT EXTRACT(MONTH FROM expense_date) AS m, COALESCE(SUM(amount), 0) AS total
            FROM expenses
